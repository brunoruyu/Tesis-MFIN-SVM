...
C_range = [15]#np.logspace(-1, 5, 9)
g_range = [0.0001]#np.logspace(-6, -2, 13)

dfOpt= pd.DataFrame(columns=['Gamma','C','Conf','Cant'])
i=0
for igamma in g_range:
    for iC in C_range:
        ...
        cant=predlist.count([-1.0])
        print("i = ",i)
        row = [igamma,iC,confidence,cant]
        print(row)
